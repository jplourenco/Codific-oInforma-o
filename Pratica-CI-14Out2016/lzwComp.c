...
*/ 
  ...
  if( (file_name = malloc(strlen(input_file_name) + strlen(".lzw") + 1)) != NULL){
    sprintf(file_name,"%s.lzw",input_file_name);
    lzw_file=fopen(file_name,"wb"); 
  } else {
...
*/ 
  ...
  { 
    ...
    unsigned int lookup_index = find_match(string_code, character);
    /* ??? */                               /* See if the string is in */ 
                                            /* the table.  If it is,   */ 
                                            /* get the code value.  If */ 
                                            /* the string is not in the*/ 
  
                                            /* table, try to add it.   */ 
  //String+Character is found;
  //
    if (code_value[lookup_index] != -1) {
          string_code = code_value[lookup_index];
      }
    ...
    /* after adding the new one*/
          ...
          //Inspired by decompress
          if (next_code <= MAX_CODE) 
            { 
              code_value[lookup_index] = next_code; 
              prefix_code[lookup_index] = string_code; 
              append_character[lookup_index] = character; 
              next_code++;
            }                               
      //ouput code for STRING after Adding
